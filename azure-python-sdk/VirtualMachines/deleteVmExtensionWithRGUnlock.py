...
rg_skip_set = {}  # if defined, this will skip VMs that are in one of these RGs

subscription_names_list = [
    "az-subscription-name-01"
]  # list of strings of the full display name of desired subscriptions

vm_name_re_filter_list = []  # if defined, this list will filter out any VMs whose name doesn't match one of the filters.
vm_extension_names_list = [
    "MDE.Linux",
    "MDE.Windows"
...
if __name__ == "__main__":
    ...
    # vm_extension_names_list = [x.lower() for x in vm_extension_names_list]
    ...
    for subscription in subscription_list:
        ...
        vm_extension_id_list = []
        for extension_name in vm_extension_names_list:
            vm_extension_id_list += jmespath.search('[].resources[? id.ends_with(@, `' + extension_name + '`)][]',
                                                    all_vm_dictionary_list)

        vm_extension_id_list = [x.get('id').lower() for x in vm_extension_id_list]
        rg_name_set = set([x.split('/')[4] for x in vm_extension_id_list]) - rg_skip_set
        ...
        for rg_name in rg_name_set:
            ...
            for extension_id in vm_extension_id_list:
