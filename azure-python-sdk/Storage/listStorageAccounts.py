    ...
    for subscription in subscription_client.subscriptions.list():
        if subscription.display_name in subscription_names:
            ...
            try:
                ...
                # storage_accounts = [account for account in storage_accounts if (account.minimum_tls_version != 'TLS1_2'
                #                                                                 or account.as_dict().get('allow_blob_public_access', True)
                #                                                                 or not account.enable_https_traffic_only)]
