...
subscription_names = ["subscription-name"]  # list of strings of the full display name of desired subscriptions
...
if __name__ == "__main__":
    ...
    with open(file=file_path, mode='w', newline='') as csvfile:
        ...
        for subscription in subscription_list:
            ...
            network_security_group_list = None
            while not network_security_group_list:
                # If Azure API gives error (like API limit), wait 10 seconds and try again.
                try:
                    network_security_group_list = network_client.network_security_groups.list("az-acflabs-nonprod-01-network-nonprod-westus-rg-01")  # , "mmull04-tru-test16-dev-westus-nsg-01")]
                    if not network_security_group_list:
                        break  # if no flow logs found, go to next subscription without retrying.
                except CloudError as e:
                    print('EXCEPTION {}'.format(e))
                    sleep(10)
            for security_group in network_security_group_list:
                flow_log_list = None
